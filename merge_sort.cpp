...
void merge(int a[], int left[], int right[], int nl, int nr)
{
    ...
    i = j = k = 0;
    //this condition is to make sure that the it iterates over the loop whenever the size of the indexes i and j are less than the maximum/total size of the subarray. (it's obvious, that index can't be higher than the actual size of array)
	while (i < nl && j < nr) 
	{
